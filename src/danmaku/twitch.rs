...
use crate::{danmaku::DMLDanmaku, dmlive::DMLContext, utils::dmlch};
...
impl Twitch {
    ...
    async fn get_ws_info(&self) -> anyhow::Result<(String, Vec<String>)> {
        ...
        reg_datas.push(format!("JOIN #{}", self.ctx.cm.room_id));
        // println!("{:?}", &reg_datas);
    ...
    pub async fn run(&self, dtx: dmlch::Sender<DMLDanmaku>) -> anyhow::Result<()> {
        let (ws, mut reg_datas) = self.get_ws_info().await?;
        ...
        let hb_task = async {
            while ws_write.send(Message::text(HEARTBEAT)).await.is_ok() {
