...
const ASS_HEADER_TEXT: &str = include_str!("header.ass");
...
impl Danmaku {
    ...
    fn get_avail_danmaku_channel(&self, c_pts: u64, len: usize) -> Option<usize> {
        ...
        for (i, c) in self.dchannels.borrow_mut().iter_mut().enumerate() {
            ...
            if (((self.ctx.cm.danmaku_speed.get() as f64 - c_pts as f64 + c.begin_pts as f64) * s) > 1920.0)
                || (((c.length + 1920) as f64 * (c_pts as f64 - c.begin_pts as f64)
    ...
    fn launch_single_danmaku(
        ...
        let mut f1 = || {
            d.color.trim().is_empty().not().then_some({})?;
    ...
    async fn launch_live_danmaku_task(&self, rx: &dmlch::Receiver<DMLDanmaku>) -> Result<()> {
        let mut socket = self.ctx.im.get_danmaku_tcp_socket().await?;
    ...
    async fn launch_video_danmaku_task(&self, rx: &dmlch::Receiver<DMLDanmaku>) -> Result<()> {
    ...
    pub async fn danmaku_client_task(&self, dtx: dmlch::Sender<DMLDanmaku>) -> Result<()> {
        loop {
            let _ = match self.ctx.cm.site {
                Site::BiliLive => {
            ...
            .map_err(|e| info!("danmaku client error: {:?}", e));
            if dtx.is_closed() {
                break;
            }
