...
impl Danmaku {
    pub fn new(cm: Arc<ConfigManager>, im: Arc<crate::ipcmanager::IPCManager>, _mtx: Sender<DMLMessage>) -> Self {
        Self {
            ...
            ratio_scale: Arc::new(RwLock::new(0.0)),
    ...
    async fn get_avail_danmaku_channel(
        ...
        for (i, c) in channels.iter_mut().enumerate() {
            if i.ge(self.channel_num.read().await.deref()) {
                break;
            }
            if ((*self.speed.read().await - c_pts + c.begin_pts) as f64 * s) > 1920.0 {
                continue;
            } else {
                if ((c.length + 1920) * (c_pts - c.begin_pts) / *self.speed.read().await) < c.length {
                    continue;
                } else {
                    c.length = len;
                    c.begin_pts = c_pts;
    ...
    async fn get_danmaku_display_length(&self, dm: &str) -> usize {
        ...
        (((fs as f64 * 0.75 * non_ascii_num as f64) + (fs as f64 * 0.25 * ascii_num as f64))
            * self.ratio_scale.read().await.deref())
        .round() as usize
    }
    ...
    async fn launch_danmaku(
        ...
        let cluster = if n.is_empty() {
            ...
            let ass = format!(
                ...
                format!(
                    "{:02x}",
                    (self.font_alpha.read().await.deref() * 255 as f64) as u8
                ),
                avail_dc * self.font_size.read().await.deref(),
                0 - display_length as isize,
                &d,
                *read_order,
                &n,
                format!("{}{}{}", &c[4..6], &c[2..4], &c[0..2]),
                self.font_size.read().await.deref(),
            )
            .into_bytes();
        ...
        let mkv_bin = bincode::DefaultOptions::new().with_big_endian().serialize(&cluster)?;
