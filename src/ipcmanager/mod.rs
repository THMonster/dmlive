...
pub struct IPCManager {
    ...
    danmaku_tcp_rx: Option<Receiver<TcpStream>>,
    stream_unix_rx: Option<Receiver<UnixStream>>,
    stream_tcp_rx: Option<Receiver<TcpStream>>,
    dashv_tcp_rx: Option<Receiver<TcpStream>>,
    dasha_tcp_rx: Option<Receiver<TcpStream>>,
}
...
impl IPCManager {
    ...
    pub async fn run(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        let mut tasks: Vec<std::pin::Pin<Box<dyn Future<Output = Result<(), Box<dyn std::error::Error>>>>>> =
            Vec::new();
    ...
    pub fn get_f2m_socket_path(&self) -> String {
        if self.plat == 0 {
            ...
            format!("http://127.0.0.1:{}", &self.f2m_port)
        }
    }
    ...
    pub async fn get_danmaku_socket(&self) -> anyhow::Result<Box<dyn DMLStream>> {
        if self.plat == 0 {
            let s = self.danmaku_unix_rx.as_ref().ok_or(anyhow!("gds err 1"))?.recv().await?;
            return Ok(Box::new(s));
        }
        Err(anyhow!("gds err 2"))
    }

    pub async fn get_stream_socket(&self) -> Result<Box<dyn DMLStream>, Box<dyn std::error::Error>> {
        if self.plat == 0 {
            let s = self.stream_unix_rx.as_ref().ok_or("gss err 1")?.recv().await?;
            return Ok(Box::new(s));
        }
    ...
    pub async fn get_video_socket(&self) -> Result<Box<dyn DMLStream>, Box<dyn std::error::Error>> {
        let s = self.dashv_tcp_rx.as_ref().ok_or("gvs err 1")?.recv().await?;
        Ok(Box::new(s))
    }

    pub async fn get_audio_socket(&self) -> Result<Box<dyn DMLStream>, Box<dyn std::error::Error>> {
        let s = self.dasha_tcp_rx.as_ref().ok_or("gas err 1")?.recv().await?;
        Ok(Box::new(s))
    }
}
